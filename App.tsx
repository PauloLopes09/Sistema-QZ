...
const App: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      ...
      setActiveMenu('acompanhamento-licitacoes'); // ID Corrigido para bater com a Sidebar
      setActiveTab(1);
    }, 2000);
  };
  ...
  return (
    <div className="flex h-screen bg-zinc-50 overflow-hidden font-sans text-zinc-900">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-violet-50 rounded-[40px] p-8 border border-violet-100 space-y-6">
                  ...
                  <select 
                    ...
                    className="w-full px-6 py-4 bg-white border border-zinc-200 rounded-2xl font-black text-zinc-800 shadow-sm outline-none focus:ring-4 focus:ring-violet-100 transition-all"
                  >
      ...
      <aside className="w-80 bg-white border-r border-zinc-200 flex flex-col z-50">
        <div className="p-10">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 bg-gradient-to-br from-violet-600 to-fuchsia-600 rounded-2xl flex items-center justify-center shadow-2xl shadow-violet-200 hover:scale-105 transition-transform">
        ...
        <nav className="flex-1 px-6 space-y-2">
          {[
            ...
            { id: 'acompanhamento-licitacoes', icon: ListTodo, label: 'Acompanhamento' },
          ].map((item) => (
        ...
        <header className="h-24 bg-white/80 backdrop-blur-md border-b border-zinc-200 px-10 flex items-center justify-between z-40">
          ...
          <div className="flex items-center gap-6">
            <div className="relative">
              ...
              <input placeholder="Pesquisar..." className="bg-zinc-100 border-none rounded-2xl pl-12 pr-6 py-3.5 text-sm w-80 font-bold focus:ring-2 focus:ring-violet-500 outline-none transition-all" />
            </div>
                ...
                <div className="bg-white rounded-[48px] border border-zinc-200 p-12 space-y-10 min-h-[500px] shadow-sm">
                  {activeTab === 1 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 animate-in fade-in">
                      ...
                      <div className="md:col-span-2"><label className="block text-sm font-black text-zinc-700 mb-3 uppercase">Objeto Detalhado *</label><textarea rows={4} value={formData.objeto} onChange={(e) => handleInputChange('objeto', e.target.value)} className="w-full px-6 py-5 bg-zinc-50 border border-zinc-200 rounded-3xl font-medium focus:ring-4 focus:ring-violet-50 outline-none resize-none" /></div>
                    </div>
                  )}
                  {activeTab === 2 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10 animate-in fade-in">
                  ...
                  <div className="flex items-center justify-between pt-10 border-t border-zinc-100">
                    ...
                    {activeTab < 3 ? (
                      <button onClick={() => setActiveTab(prev => Math.min(3, prev + 1))} className="px-10 py-4 bg-zinc-900 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-xl shadow-zinc-200 hover:scale-105 transition-all">Próximo Passo</button>
                    ) : (
                      <button onClick={handleSave} className="px-12 py-4 bg-violet-600 text-white font-black uppercase text-[10px] tracking-widest rounded-2xl shadow-2xl shadow-violet-200 flex items-center gap-3 hover:scale-105 transition-all hover:bg-violet-700"><Save className="w-5 h-5" /> Salvar & Iniciar Acompanhamento</button>
                    )}
                  </div>
                </div>
              </div>
                      ...
                      <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-zinc-50">
                          {tenders.length === 0 ? (
                            ...
                            tenders.map((tender) => (
                              <tr key={tender.id} className="hover:bg-zinc-50/50 group transition-all">
                                ...
                                <td className="p-8">
                                  <span className={`inline-block px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest ${
                                    ...
                                    tender.statusAtual === TenderStatus.SUSPENSO ? 'bg-rose-100 text-rose-700' : 'bg-zinc-100 text-zinc-600'
                                ...
                                <td className="p-8 text-right">
                                  <div className="flex items-center justify-end gap-3">
                                    ...
                                    <button onClick={() => setManagingTender(tender)} className="px-8 py-3.5 bg-zinc-900 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-violet-600 opacity-0 group-hover:opacity-100 transition-all flex items-center gap-2">Gerenciar <ArrowRightCircle className="w-4 h-4" /></button>
                                  </div>
                                </td>
                              </tr>
                            ))
                           ...
                           return (
                             <div key={day} className={`p-4 min-h-32 border-r border-b border-zinc-100 relative group transition-all hover:bg-zinc-50/80 ${isToday ? 'bg-violet-50/30' : ''}`}>
                               ...
                               <div className="mt-3 space-y-2">
                                 {aberturas.map((t, idx) => (
                                   <div key={`ab-${idx}`} onClick={() => setManagingTender(t)} className="cursor-pointer flex flex-col p-2 bg-violet-600 text-white rounded-xl shadow-lg shadow-violet-100 hover:scale-105 transition-all">

...
export const Dashboard: React.FC<DashboardProps> = ({ tenders }) => {
  ...
  const getEventsForDay = (day: number) => {
    ...
    return { aberturas: tenders.filter(t => t.dataAbertura === dateStr) };
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white rounded-[40px] border border-slate-200 shadow-sm overflow-hidden flex flex-col min-h-[700px]">
          ...
          <div className="grid grid-cols-7 flex-1">
            {Array.from({ length: firstDayOfMonth }).map((_, i) => <div key={`empty-${i}`} className="p-2 border-r border-b border-slate-50 bg-slate-50/10" />)}
            {Array.from({ length: daysInMonth }).map((_, i) => {
              const day = i + 1;
              const { aberturas } = getEventsForDay(day);
              const isToday = day === now.getDate() && month === now.getMonth() && year === now.getFullYear();
              return (
                <div key={day} className={`p-2 min-h-32 border-r border-b border-slate-100 transition-all ${isToday ? 'bg-blue-50/30' : ''}`}>
                  <span className={`text-xs font-black ${isToday ? 'text-blue-600 bg-blue-100 rounded-full px-2 py-0.5' : 'text-slate-400'}`}>{day}</span>
          ...
          <div className="bg-white rounded-[40px] border border-slate-200 shadow-sm p-8 h-full">
            ...
            <div className="space-y-4">
              {upcomingTenders.map((t, idx) => (
                <div key={idx} className="p-4 bg-slate-50 rounded-3xl border border-slate-100 group">
                  <div className="flex gap-4 mb-3">
                    <div className={`w-12 h-12 shrink-0 rounded-2xl flex flex-col items-center justify-center border border-slate-100 shadow-sm ${getTenderStatus(t) === 'ok' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-200">
                    <span className="text-[9px] font-black text-blue-600 uppercase flex items-center gap-1"><Clock className="w-3 h-3" />{t.horarioSessao || '00:00'}</span>
                    <span className={`text-[9px] font-black px-2 py-0.5 rounded uppercase ${getTenderStatus(t) === 'ok' ? 'text-emerald-600 bg-emerald-100' : 'text-rose-600 bg-rose-100'}`}>{getTenderStatus(t) === 'ok' ? 'Habilitado' : 'Pendente'}</span>
                  </div>
                </div>
              ))}
            </div>
